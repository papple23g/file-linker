...
- 修復 hover command 連結，讓方括號檔名點擊能穩定觸發。
- 新增 Everything UTF-8 匯出結果處理，支援中文與 emoji 檔名，同時保留舊版 stdout fallback。
- 恢復 Windows 以系統預設程式開啟檔案，例如 `.pptx` 會交給 PowerPoint 或使用者設定的預設程式。
- 恢復單元測試與 extension-host 整合測試，覆蓋點擊、搜尋結果與預設程式開啟流程。
